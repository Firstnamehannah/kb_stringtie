...
# TEST TEST TEST
        ...
        #kncls.wsClient.create_workspace({'workspace': cls.wsName})

        # use existing Narrative workspace + existing alignment refs
        cls.wsName = "keenholdhc:narrative_1782222319261"
        cls.alignment_ref_1 = "77421/17/1"
        cls.alignment_ref_2 = "77421/18/1"
    ...
    def prepare_ama_data(cls):
        ...
        ret = cls.kb_stringtie_runner.run_stringtie_app({
            'alignment_object_ref':"77421/17/1",
            'workspace_name': "keenholdhc:narrative_1782222319261",
    ...
    # added tests for long read mode and mix mode
    def test_StringTieUtil_generate_command_long_read_mode(self):
        command_params = {
            'output_transcripts': 'out.gtf',
            'gene_abundances_file': 'genes.tsv',
            'input_file': 'long_reads.bam',
            'long_read_mode': True
        }
        ...
        self.assertIn('long_reads.bam', command)
        self.assertNotIn('--mix', command)

    def test_StringTieUtil_generate_command_mix(self):
        command_params = {
            'output_transcripts': 'out.gtf',
            'gene_abundances_file': 'genes.tsv',
            'input_file': 'short_reads.bam',
            'long_read_input_file': 'long_reads.bam',
            'mix_mode': True
        }
        ...
        self.assertIn('short_reads.bam', command)
        self.assertIn('long_reads.bam', command)
        self.assertNotIn('-L', command)

    def test_StringTieUtil_generate_command_mix_missing_long_read_file(self):
        command_params = {
            'output_transcripts': 'out.gtf',
            'gene_abundances_file': 'genes.tsv',
            'input_file': 'short_reads.bam',
        ...
        with self.assertRaisesRegex(
            ...
            self.stringtie_runner._generate_command(command_params)

    def test_StringTieUtil_generate_command_mix_and_long_read_mode(self):
        command_params = {
            'output_transcripts': 'out.gtf',
            'gene_abundances_file': 'genes.tsv',
            'input_file': 'short_reads.bam',
    ...
    def test_run_stringtie_app_mix_missing_long_read_alignment(self):
        input_params = {
            'alignment_object_ref': self.alignment_ref_1,
            'workspace_name': self.getWsName(),
            'expression_suffix': '_expression',
    ...
    def test_run_stringtie_app_mix_and_long_read_mode(self):
        input_params = {
            'alignment_object_ref': self.alignment_ref_1,
            'workspace_name': self.getWsName(),
            'expression_suffix': '_expression',
            ...
            'mix_mode': 1,
            'long_read_mode': 1,
            'long_read_alignment_object_ref': self.alignment_ref_2
        }
    ...
    def test_run_stringtie_app_mix_with_long_read_alignment(self):
      # inspect the alignment objects
        # info = self.ws.get_object_info3({
        #     "objects": [
        #         {"ref": self.alignment_ref_1},
        #         {"ref": self.alignment_ref_2}
        #     ]
        # })["infos"]

        # print("SHORT READ OBJECT INFO:", info[0])
        # print("LONG READ OBJECT INFO:", info[1])

        short_set = self.ws.get_objects2({
            "objects": [{"ref": self.alignment_ref_1}]
        })["data"][0]["data"]

        long_set = self.ws.get_objects2({
            "objects": [{"ref": self.alignment_ref_2}]
        })["data"][0]["data"]

        print("SHORT SET DATA:", json.dumps(short_set, indent=2))
        print("LONG SET DATA:", json.dumps(long_set, indent=2))

        input_params = {
            "alignment_object_ref": self.alignment_ref_1,
            "long_read_alignment_object_ref": self.alignment_ref_2,
            "workspace_name": self.getWsName(),
            "expression_suffix": "_mix_expression",
            "expression_set_suffix": "_mix_expression_set",
            "mix_mode": 1,
            "long_read_mode": 0,
            "num_threads": 2
        }
